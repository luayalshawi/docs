...
**إضافة منتج أو خدمة في منصة حزمة تك**

1. **إدخال البيانات الأساسية**
   - اسم المنتج أو الخدمة.
   - اختيار ما إذا كان قابل للبيع، الشراء، أو في نقطة البيع.
   - إضافة صورة للتعريف السريع.
2. **تحديد النوع**
   - **قابل للاستهلاك**: لا يحتاج تخزين.
   - **خدمة**: استشارة، صيانة، إلخ.
   - **قابل للتخزين**: يتم تتبع المخزون.
3. **الأسعار والضرائب**
   - تحديد سعر البيع.
   - اختيار إعادة فوترة النفقات (لا / التكلفة / سعر البيع).
   - تحديد الضريبة (شامل السعر أو غير شامل).
4. **وحدات القياس والتصنيف**
   - اختيار وحدة قياس رئيسية (مم، سم، جرام...).
   - تحديد فئة نقطة البيع ووحدة الشراء عند الحاجة.
5. **إضافة الباركود**
   - إدخال الباركود أو إنشاؤه تلقائياً.
6. **الحفظ**
   - اضغط على **حفظ** لتطبيق جميع التعديلات.